...
class ContentFile:
    ...
    def needs_update(self, lang=None):
        ...
        git_ls = sp.Popen(
            shlex.split(f'git ls-files -z {" ".join(str(t) for t in trns)}'),
            stdout=sp.PIPE
        )
        ...
        git_ls.wait()
        my_timestamp = timestamps[str(self.path.relative_to(Path.cwd()))]
...
def scan_content_at_dir(cfg, path):
    for f in path.rglob('*.md'):
        ...
        if cfg.is_excluded(cf.path):
            logger.debug(
                "File excluded: '%s'", cf.path.relative_to(cfg.root_dir)
            )
            continue
        yield cf
